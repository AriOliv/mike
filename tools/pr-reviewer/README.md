...
Beyond PR review, two more Actions workflows ship with the tool.

### issue-to-pr

`issue_to_pr.py` (workflow: `issue-to-pr.yml`) turns a newly opened issue into a
pull request. On `issues: opened` (or an `auto-pr` label) it asks Gemini for the
file changes that resolve the issue, writes them to an `auto-fix/issue-<N>`
branch, and opens a PR that closes the issue.

- Files under `.github/workflows/` are **skipped**: the Actions `GITHUB_TOKEN`
  cannot push workflow files (that needs a PAT with the `workflow` scope), and
  auto-committing runnable CI is a security risk. When a generation touches only
  workflow files, the bot comments on the issue and opens no PR.
- Required secret: `GEMINI_API_KEY`. Permissions: `contents: write`,
  `pull-requests: write`, `issues: write`.

### cybersecurity-analysis

`security_scan.py` (workflow: `cybersecurity-analysis.yml`) audits the **whole
repository** (not a single diff) with the same pluggable review skill. It runs
on manual dispatch, a weekly schedule, and pushes to `main`, and writes the
findings to the Actions job summary.

- **Reporting**: on `push`/`schedule`/dispatch it maintains a single tracking
  issue ("🔒 Security scan report"), updating it in place; on `pull_request` it
  scans the PR head and upserts one PR comment. Toggle with `REPORT_ISSUE` /
  `REPORT_PR_COMMENT`.
- **Fallback**: if the managed-agent sandbox is unavailable, it falls back to a
  plain Gemini model (`GEMINI_MODEL`, default `gemini-3.6-flash`) with the
  repository source inlined, so the scan still runs.
- Set `FAIL_ON` (e.g. `high`) to fail the job on findings at or above a
  severity; the default `never` reports without failing the build.
...
| `security_scan.py` | Whole-repo security scan (see [Additional workflows](#additional-workflows)) |
| `issue_to_pr.py` | Issue → PR generator (see [Additional workflows](#additional-workflows)) |
| `github_client.py` | GitHub REST helpers (PR and files, diff prompt, comments/issues, Basic auth header) |
